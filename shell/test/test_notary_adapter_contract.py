...
def _svc_stub(name):
    if name == "case_workspace":
        return case_workspace
    if name == "word_batch_export":
        return word_batch_export
    pytest.fail(f"unexpected _svc({name!r})")
...
def adapter_db(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(notary_adapter, "_svc", _svc_stub)
    sess = Session()
    yield sess
    sess.close()
