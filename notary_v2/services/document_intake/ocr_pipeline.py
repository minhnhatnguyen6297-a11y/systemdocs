"""Qwen OCR pipeline — deterministic parse/pair machinery for document intake.

Moved verbatim from routers/ocr_ai.py (MIN-108) so the web route, the
multi-source intake service and the Electron sidecar share one implementation.
ocr_ai.py re-exports every name from here, giữ nguyên surface `ocr_ai.<name>`
cho tests, routers/zalo_inbox.py và sidecar adapter.

Flow: image bytes -> Qwen native OCR (raw text lines) -> deterministic parse
(CCCD person / GCN property / death certificate) -> pair front+back -> docs.
AI is text-only OCR; all field parsing is deterministic here.
"""
...
async def process_image_bytes(
    ...
    *,
    ...
    """OCR mot anh da doc san thanh doc — loi chung cho web route va intake.

    `ocr_call` injectable: production dung _call_qwen_native_ocr_single; tests
    truyen fake tra list[str] lines. Tra dict
    {filename, raw_lines, ai_doc, error, error_stage}.
    """
    call = ocr_call or _call_qwen_native_ocr_single
    lines: list[str] = []
    try:
        if not api_key:
...
async def process_property_image_bytes(
    file_bytes: bytes,
    filename: str,
    *,
    model: str,
    ...
    """OCR anh giay to dat voi rotate retry + footer date rescue (web path)."""
    call = ocr_call or _call_qwen_native_ocr_single
    retry_check = should_retry or _should_retry_property_rotate
    if not api_key:
        raise HTTPException(status_code=500, detail="Missing API key for OCR AI model")
